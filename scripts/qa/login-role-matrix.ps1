...
$accounts = @(
    ...
    @{ User = 'a_core'; Role = 'qa_research_member'; Must = @('task:submission:add', 'fund:use:list'); MustNot = @('task:framework:add', 'task:submission:audit') },
    @{ User = 'a_member'; Role = 'qa_research_member'; Must = @('task:submission:add', 'fund:use:list'); MustNot = @('task:framework:add', 'task:submission:audit') },
    @{ User = 'a_expert'; Role = 'qa_research_member'; Must = @('task:submission:add', 'fund:use:list'); MustNot = @('task:framework:add', 'task:submission:audit') },
    @{ User = 'b_leader'; Role = 'qa_research_leader'; Must = @('task:framework:add', 'task:submission:audit'); MustNot = @('fund:budget:add') },
    @{ User = 'b_core'; Role = 'qa_research_member'; Must = @('task:submission:add'); MustNot = @('task:framework:add') },
